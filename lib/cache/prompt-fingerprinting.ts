 ...
 */
export function hasBridgePromptInConversation(input: any[] | undefined, bridgeContent: string): boolean {
...
function extractTextContent(content: any): string | null {
	...
	if (Array.isArray(content)) {
		const textItems = content.filter((item) => item.type === "input_text" && item.text);
 ...
 */
export function getCachedBridgeDecision(inputHash: string, toolCount: number): BridgeCacheEntry | null {
 ...
 */
export function cacheBridgeDecision(inputHash: string, toolCount: number, shouldAddBridge: boolean): void {
