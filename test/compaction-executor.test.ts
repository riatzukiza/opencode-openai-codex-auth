...
import {
	...
	finalizeCompactionResponse,
} from "../lib/compaction/compaction-executor.js";
...
describe("Compaction executor", () => {
	it("rewrites auto compaction output, metadata, and persists summary", async () => {
		...
		const decision: CompactionDecision = {
			mode: "auto",
			reason: "token limit",
			preservedSystem: [{ type: "message", role: "system", content: "system instructions" }],
			serialization: {
	...
	it("does not add auto note when compaction is command-based", async () => {
		const payload = {
			output: [
				{
					role: "assistant",
					content: [{ type: "output_text", text: "Previous might" }],
				},
			],
			metadata: {},
		};
