...
* To use the **tool-call remap** prompt instead, set **`CODEX_MODE=0`** (or put `"codexMode": false` in `~/.opencode/openhax-codex-config.json`).
...
## full **call_id memory** remapper (a227eb4 behavior)
