...
- **Evidence**: `index.ts:123-125` instantiates `SessionManager` with `enabled = pluginConfig.enablePromptCaching ?? false`, so caching is *disabled* unless users flip a config switch. Codex CLI always attaches a `prompt_cache_key` (`codex-rs/core/src/client.rs`, `ResponsesApiRequest` builder) and therefore guarantees cache reuse.
